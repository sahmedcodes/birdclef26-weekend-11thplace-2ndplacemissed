...
class Config:
    ...
    experiment_params = {
        "output_dir" : "outputs",
        "experiment_name" : "stage3_b0ns_v3",
        "fold" : 0,
        "save_epochs" : [24, 34, 49, 74, 84, 94]
    }
...
class BirdClEF26Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.xc_df = pd.read_csv(os.path.join("../stage2/predictions_xc_1", "prediction_xc_sp.csv"))
        self.ss_df = pd.read_csv(os.path.join("../stage2/predictions_ss_1", "prediction_ss_ignored_sp.csv"))

    
    def __len__(self):
        return len(self.df) + len(self.xc_df) + len(self.ss_df)
    ...
    def load_xc_sample(self, filepath, sample_rate=32_000, res_type="kaiser_best"):
        wave, _ = librosa.load(filepath, sr=sample_rate, res_type=res_type, mono=True)
        max_len = self.duration * self.sample_rate
    ...
    def load_xc(self, idx):

        wave = self.load_xc_sample(
                filepath=self.xc_df["ss_fp"].iloc[idx],
        ...
        try:
            xps = np.load(os.path.join("../stage2/predictions_xc_1", self.xc_df["fp_w"].iloc[idx]))
            FRAME_WISE_PAD = 1
            ...
            print (xps.shape, ps.shape, start, end, wave_len)
    ...
    def load_ss(self, idx):
        ...
        start = start // self.sample_rate
        ps = np.load(os.path.join("../stage2/predictions_ss_1", self.ss_df["fp_w"].iloc[idx]))
        FRAME_WISE_PAD = 1
        start = start - FRAME_WISE_PAD
        if start < 0:
            start = 0
    ...
    def __getitem__(self, idx):
        ...
        main_len = len(self.df)
        xc_len = len(self.xc_df)
        ss_len = len(self.ss_df)
        
        if idx < main_len:
            wave, target = self.load_record(idx)

        elif idx < main_len + xc_len:
            xc_idx = idx - main_len
            wave, target = self.load_xc(xc_idx)

        elif idx < main_len + xc_len + ss_len:
            ss_idx = idx - main_len - xc_len
            wave, target = self.load_ss(ss_idx)
