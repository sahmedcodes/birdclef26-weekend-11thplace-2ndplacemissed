...
class Config:
    ...
    experiment_params = {
        "output_dir" : "outputs",
        "experiment_name" : "stage2_b2ns_v2",
        "fold" : 0,
...
class SSDataset(torch.utils.data.Dataset):
    def __init__(self):
        self.ss_df = pd.read_csv(os.path.join("../stage2/predictions_ss", "prediction_ss_ignored.csv"))
    ...
    def load_ss_sample(self, idx):
        ...
        ps = np.load(os.path.join("../stage2/predictions_ss", self.ss_df["fp_w"].iloc[idx]))
        FRAME_WISE_PAD = 0
...
class SEDModel(nn.Module):
    ...
    def calculate_loss(self, output, insects_targets, amph_targets, mam_targets, aves_targets, criterion):
        ...
        return loss, np.array(clip_losses_items), np.array(frame_losses_items)
...
def select_size(inputs, size, device):
    inp = []
    for i in inputs:
        inp.append(i[size[0]:size[1]].to(device))
    return inp

def train_forward_pass(model, inputs, scaler, criterion, device, size):
    wave, targets = inputs
    wave = select_size([wave], size, device)[0]
    targets = select_size(targets, size, device)
    insects_targets, amph_targets, mam_targets, aves_targets = targets
...
def process_batch(batch):
    ...
    return wave, [insects_targets, amph_targets, mam_targets, aves_targets]
...
def train_epoch(model, loader, optimizer, scaler, params):
    ...
    for step, batch in bar:
        ...
        loss1, clip_loss1, frame_loss1 = train_forward_pass(
            model = model,
            inputs=inputs,
            scaler=scaler,
            criterion=criterion,
        ...
        loss_value1 = loss1.detach().cpu().item()


        loss2, clip_loss2, frame_loss2 = train_forward_pass(
            model = model,
            inputs=inputs,
            scaler=scaler,
            criterion=criterion,
        ...
        loss_value2 = loss2.detach().cpu().item()
        loss_value = (loss_value1 + loss_value2)
        
        
        clip_loss = ((clip_loss1 + clip_loss2) / 2.0).tolist()
        frame_loss = ((frame_loss1 + frame_loss2) / 2.0).tolist()
