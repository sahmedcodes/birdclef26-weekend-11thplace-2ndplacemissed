...
class Config:
    ...
    experiment_params = {
        "output_dir" : "outputs",
        "experiment_name" : "stage0_b0ns_v2",
        "fold" : 0
    }
...
class BirdClEF26Dataset(torch.utils.data.Dataset):
    ...
    def pick_random_sample(self, wave, duration_samples):
        ...
        return wave[start : start + duration_samples]
    ...
    def sample_wave(self, wave, duration, sampling_type):
        ...
        wave = self.pad_if_needed(wave, expected_len, pad_type="left")
        if sampling_type == "rms":
            sample = self.pick_rms_sample(wave, expected_len)
        elif sampling_type == 'random':
            sample = self.pick_random_sample(wave, expected_len)
        else:
